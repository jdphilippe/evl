...
function tie_shortcode_flickr( $atts, $content = null ) {
	...
	<script type="text/javascript" src="https://www.flickr.com/badge_code_v2.gne?count='. $number .'&amp;display='. $orderby .'&amp;size=s&amp;layout=x&amp;source=user&amp;user='. $id .'"></script>
	</div>';
...
function tie_facebook_shortcode( $atts, $content = null ) {
	global $post;
    return '<iframe src="https://www.facebook.com/plugins/like.php?href='. get_permalink($post->ID) .'&amp;layout=box_count&amp;show_faces=false&amp;width=100&amp;action=like&amp;font&amp;colorscheme=light&amp;height=65" scrolling="no" frameborder="0" style="border:none; overflow:hidden; width:50px; height:65px;" allowTransparency="true"></iframe>';
}
...
function tie_tweet_shortcode( $atts, $content = null ) {
	global $post;
    return '<a href="https://twitter.com/share" class="twitter-share-button" data-url="'. get_permalink($post->ID) .'" data-text="'. get_the_title($post->ID) .'" data-via="'. tie_get_option( 'share_twitter_username' ) .'" data-lang="en" data-count="vertical" >tweet</a><script type="text/javascript" src="https://platform.twitter.com/widgets.js"></script>';
}
...
</script><a class='DiggThisButton DiggMedium' href='https://digg.com/submit?url". get_permalink($post->ID) ."=&amp;title=". get_the_title($post->ID) ."'></a>";
}
...
function tie_pinterest_shortcode( $atts, $content = null ) {
	...
	<a href="https://pinterest.com/pin/create/button/?url='.get_permalink($post->ID).'&amp;media='.tie_thumb_src( 'slider' ).' class="pin-it-button" count-layout="vertical"><img border="0" src="//assets.pinterest.com/images/PinExt.png" title="Pin It" /></a>';
...
function tie_feedburner_shortcode( $atts, $content = null ) {
    @extract($atts);
    return '<a href="https://feeds.feedburner.com/'.$name.'"><img src="https://feeds.feedburner.com/~fc/'.$name.'?anim=1" height="26" width="88" style="border:0" alt="" /></a>';
}
